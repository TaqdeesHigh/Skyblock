<?php

declare(strict_types=1);

namespace taqdees\Skyblock\crafting\items\tools\stone;

use pocketmine\item\VanillaItems;
use pocketmine\block\VanillaBlocks;
use taqdees\Skyblock\crafting\Recipe;

class StoneAxeRecipe extends Recipe {
    
    protected function initializeRecipe(): void {
        $this->setName("Stone Axe");
        $this->setDescription([
            "§7An improved chopping tool.",
            "§7Faster than wooden tools."
        ]);

        $this->setPattern([
            [VanillaBlocks::COBBLESTONE()->asItem(), VanillaBlocks::COBBLESTONE()->asItem(), null],
            [VanillaBlocks::COBBLESTONE()->asItem(), VanillaItems::STICK(), null],
            [null, VanillaItems::STICK(), null]
        ]);

        $this->setResult(VanillaItems::STONE_AXE());
    }
}