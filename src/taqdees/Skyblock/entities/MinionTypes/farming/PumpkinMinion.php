...
class PumpkinMinion extends BaseMinion {
    ...
    protected function canWorkOnBlock(Vector3 $blockPos): bool {
        ...
        return $block->getTypeId() === VanillaBlocks::PUMPKIN()->getTypeId();
    }
    ...
    protected function doWork(): void {
        ...
        if ($block->getTypeId() === VanillaBlocks::PUMPKIN()->getTypeId()) {
            $world->setBlockAt((int)$blockPos->x, (int)$blockPos->y, (int)$blockPos->z, VanillaBlocks::AIR());
            $pumpkin = VanillaBlocks::PUMPKIN()->asItem();
            $added = $this->addItemToInventory($pumpkin);
            if (!$added) {
                $world->dropItem($blockPos, $pumpkin);
            }
        ...
        for ($x = -$this->workRadius; $x <= $this->workRadius; $x++) {
            for ($z = -$this->workRadius; $z <= $this->workRadius; $z++) {
                ...
                if ($block->getTypeId() === VanillaBlocks::AIR()->getTypeId()) {
                    $world->setBlockAt((int)$blockPos->x, (int)$blockPos->y, (int)$blockPos->z, VanillaBlocks::PUMPKIN());
                }
            }
        }
    }
