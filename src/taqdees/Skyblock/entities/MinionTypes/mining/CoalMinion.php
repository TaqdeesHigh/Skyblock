...
class CoalMinion extends BaseMinion {
    ...
    protected function canWorkOnBlock(Vector3 $blockPos): bool {
        ...
        return $block->getTypeId() === VanillaBlocks::COAL_ORE()->getTypeId();
    }
    ...
    protected function doWork(): void {
        ...
        if ($block->getTypeId() === VanillaBlocks::COAL_ORE()->getTypeId()) {
            $world->setBlockAt((int)$blockPos->x, (int)$blockPos->y, (int)$blockPos->z, VanillaBlocks::AIR());
            $coal = VanillaItems::COAL();
            $added = $this->addItemToInventory($coal);
            if (!$added) {
                $world->dropItem($blockPos, $coal);
            }
        ...
        for ($x = -$this->workRadius; $x <= $this->workRadius; $x++) {
            for ($z = -$this->workRadius; $z <= $this->workRadius; $z++) {
                ...
                if ($block->getTypeId() === VanillaBlocks::AIR()->getTypeId()) {
                    $world->setBlockAt((int)$blockPos->x, (int)$blockPos->y, (int)$blockPos->z, VanillaBlocks::COAL_ORE());
                }
            }
        }
    }
