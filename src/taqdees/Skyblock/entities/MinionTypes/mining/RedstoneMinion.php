...
class RedstoneMinion extends BaseMinion {
    ...
    protected function canWorkOnBlock(Vector3 $blockPos): bool {
        ...
        return $block->getTypeId() === VanillaBlocks::REDSTONE_ORE()->getTypeId();
    }
    ...
    protected function doWork(): void {
        ...
        if ($block->getTypeId() === VanillaBlocks::REDSTONE_ORE()->getTypeId()) {
            $world->setBlockAt((int)$blockPos->x, (int)$blockPos->y, (int)$blockPos->z, VanillaBlocks::AIR());
            $redstone = VanillaItems::REDSTONE_DUST()->setCount(4);
            $added = $this->addItemToInventory($redstone);
            if (!$added) {
                $world->dropItem($blockPos, $redstone);
            }
        ...
        for ($x = -$this->workRadius; $x <= $this->workRadius; $x++) {
            for ($z = -$this->workRadius; $z <= $this->workRadius; $z++) {
                ...
                if ($block->getTypeId() === VanillaBlocks::AIR()->getTypeId()) {
                    $world->setBlockAt((int)$blockPos->x, (int)$blockPos->y, (int)$blockPos->z, VanillaBlocks::REDSTONE_ORE());
                }
            }
        }
    }
