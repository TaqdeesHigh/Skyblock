...
class GoldMinion extends BaseMinion {
    ...
    protected function canWorkOnBlock(Vector3 $blockPos): bool {
        ...
        return $block->getTypeId() === VanillaBlocks::GOLD_ORE()->getTypeId();
    }
    ...
    protected function doWork(): void {
        ...
        if ($block->getTypeId() === VanillaBlocks::GOLD_ORE()->getTypeId()) {
            ...
            $added = $this->addItemToInventory($rawGold);
            if (!$added) {
                $world->dropItem($blockPos, $rawGold);
            }
        ...
        for ($x = -$this->workRadius; $x <= $this->workRadius; $x++) {
            for ($z = -$this->workRadius; $z <= $this->workRadius; $z++) {
                ...
                if ($block->getTypeId() === VanillaBlocks::AIR()->getTypeId()) {
                    $world->setBlockAt((int)$blockPos->x, (int)$blockPos->y, (int)$blockPos->z, VanillaBlocks::GOLD_ORE());
                }
            }
        }
    }
