...
class DarkOakMinion extends BaseMinion {
    ...
    protected function canWorkOnBlock(Vector3 $blockPos): bool {
        ...
        return $block->getTypeId() === VanillaBlocks::DARK_OAK_LOG()->getTypeId();
    }
    ...
    protected function doWork(): void {
        ...
        if ($block->getTypeId() === VanillaBlocks::DARK_OAK_LOG()->getTypeId()) {
            ...
            $added = $this->addItemToInventory($darkOakLog);
            if (!$added) {
                $world->dropItem($blockPos, $darkOakLog);
            }
        ...
        for ($x = -$this->workRadius; $x <= $this->workRadius; $x++) {
            for ($z = -$this->workRadius; $z <= $this->workRadius; $z++) {
                ...
                if ($block->getTypeId() === VanillaBlocks::AIR()->getTypeId()) {
                    $world->setBlockAt((int)$blockPos->x, (int)$blockPos->y, (int)$blockPos->z, VanillaBlocks::DARK_OAK_LOG());
                }
            }
        }
    }
